...
pub(crate) fn init() {
	...
	{
		// Keep enough headroom for stack allocations and DMA while leaving the
		// BitNet CPU model buffer available to the application heap.
		#[cfg(not(feature = "mman"))]
		let stack_reserve: usize = 512 * 1024 * 1024;
		...
		#[cfg(not(feature = "mman"))]
		let virt_size: usize = avail_mem
			.saturating_sub(stack_reserve)
			.align_down(LargePageSize::SIZE as usize);
		#[cfg(feature = "mman")]
