...
unsafe fn detect_from_fdt() -> Result<(), ()> {
	...
	reserve(fdt_region);
	TOTAL_MEMORY.fetch_sub(fdt_region.len().get(), Ordering::Relaxed);

	if let Some(model_node) = fdt.find_all_nodes("/hermit,model").next() {
		if let Some(mut reg) = model_node.reg() {
			if let Some(m) = reg.next() {
				let start = m.starting_address.expose_provenance();
				let size = m.size.unwrap();
				let reservation = PageRange::new(start, start + size).unwrap();
				reserve(reservation);
				TOTAL_MEMORY.fetch_sub(reservation.len().get(), Ordering::Relaxed);
				info!("Reserved model memory: {reservation:#x?} (Subtracted from total RAM)");
			}
		}
	}
