...
pub extern "C" fn sys_socket(domain: i32, type_: i32, protocol: i32) -> i32 {
	...
	if (domain == Af::Inet || domain == Af::Inet6)
		&& (sock == Sock::Stream || sock == Sock::Dgram || sock == Sock::Raw)
	{
		let mut guard = NIC.lock();
		...
		if sock == Sock::Raw {
			let smoltcp_proto = match proto {
				Ipproto::Ip | Ipproto::Icmp => smoltcp::wire::IpProtocol::Icmp,
				Ipproto::Ipv6 => smoltcp::wire::IpProtocol::Icmpv6,
				Ipproto::Tcp => smoltcp::wire::IpProtocol::Tcp,
				Ipproto::Udp => smoltcp::wire::IpProtocol::Udp,
			};
			let handle = nic.create_raw_handle(smoltcp_proto).unwrap();
			drop(guard);
			let mut socket = crate::fd::socket::raw::Socket::new(handle, smoltcp_proto);
...
pub unsafe extern "C" fn sys_setsockopt(
	...
	if level == SOL_SOCKET {
		match optname {
			...
			SO_RCVTIMEO | SO_SNDTIMEO => {
				if optlen < u32::try_from(size_of::<timeval>()).unwrap() {
					return -i32::from(Errno::Inval);
				}
				...
				let tv = unsafe { *optval.cast::<crate::time::timeval>() };
				let timeout = if tv.tv_sec == 0 && tv.tv_usec == 0 {
					None
				} else {
					Some(core::time::Duration::new(
						tv.tv_sec.try_into().unwrap(),
						(tv.tv_usec * 1000).try_into().unwrap(),
					))
				};

				let socket_option = if optname == SO_RCVTIMEO {
					SocketOption::ReadTimeout(timeout)
				} else {
					SocketOption::WriteTimeout(timeout)
				};

				let obj = get_object(fd);
				return obj.map_or_else(
					|e| -i32::from(e),
					|v| {
						block_on(
							async { v.write().await.setsockopt(socket_option).await },
							None,
						)
						.map_or_else(|e| -i32::from(e), |()| 0)
					},
	...
	let Ok(Ok(ipproto_level)) = u8::try_from(level).map(Ipproto::try_from) else {
		return -i32::from(Errno::Inval);
	};

	if ipproto_level == Ipproto::Tcp
		&& optname == TCP_NODELAY
...
pub unsafe extern "C" fn sys_getsockopt(
	...
	if level == SOL_SOCKET {
		match optname {
			SO_RCVTIMEO | SO_SNDTIMEO => {
				...
				let socket_option = if optname == SO_RCVTIMEO {
					SocketOption::ReadTimeout(None)
				} else {
					SocketOption::WriteTimeout(None)
				};
				...
				return obj.map_or_else(
					...
					|v| {
						block_on(
							async { v.write().await.getsockopt(socket_option).await },
							None,
						)
						.map_or_else(
							|e| -i32::from(e),
							|res| {
								let timeout = match res {
									SocketOption::ReadTimeout(t) => t,
									SocketOption::WriteTimeout(t) => t,
									_ => unreachable!(),
								};

								let tv = match timeout {
									Some(t) => crate::time::timeval {
										tv_sec: t.as_secs().try_into().unwrap(),
										tv_usec: (t.subsec_micros() as i32),
									},
									None => crate::time::timeval {
										tv_sec: 0,
										tv_usec: 0,
									},
								};
								...
								unsafe {
									*optval.cast::<crate::time::timeval>() = tv;
								}
	...
	if ipproto_level == Ipproto::Tcp && optname == TCP_NODELAY {
		if optval.is_null() || optlen.is_null() {
			return -i32::from(Errno::Inval);
		}
			...
			|v| {
				block_on(
					async {
						v.write()
							.await
							.getsockopt(SocketOption::TcpNoDelay(false))
							.await
					},
					None,
				)
				.map_or_else(
					|e| -i32::from(e),
					|res| {
						if let SocketOption::TcpNoDelay(value) = res {
							if value {
								*optval = 1;
							} else {
								*optval = 0;
...
pub unsafe extern "C" fn sys_sendto(
	...
	obj.map_or_else(
		...
		|v| {
			let timeout =
				block_on(async { Ok(v.read().await.write_timeout().await) }, None).unwrap();
			let res = block_on(
				async { v.read().await.sendto(slice, endpoint).await },
				timeout,
			);
			if let Err(ref e) = res {
				error!("sys_sendto: failed with error {:?}", e);
			}
			res.map_or_else(
				|e| isize::try_from(-i32::from(e)).unwrap(),
				|v| v.try_into().unwrap(),
...
pub unsafe extern "C" fn sys_recvfrom(
	...
	obj.map_or_else(
		...
		|v| {
			let timeout =
				block_on(async { Ok(v.read().await.read_timeout().await) }, None).unwrap();
			block_on(async { v.read().await.recvfrom(slice).await }, timeout).map_or_else(
				|e| isize::try_from(-i32::from(e)).unwrap(),
				|(len, endpoint)| {
					if !addr.is_null() && !addrlen.is_null() {
