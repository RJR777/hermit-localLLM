...
pub extern "C" fn sys_malloc(size: usize, align: usize) -> *mut u8 {
	...
	if ptr.is_null() && size >= 1024 * 1024 {
		warn!("__sys_malloc failed for large allocation size {size:#x}, align {align:#x}");
	}
...
#[hermit_macro::system]
#[unsafe(no_mangle)]
pub extern "C" fn sys_get_model_ptr() -> *const u8 {
	SYS.get_model_ptr()
}

#[hermit_macro::system]
#[unsafe(no_mangle)]
pub extern "C" fn sys_get_model_len() -> usize {
	SYS.get_model_len()
}

#[hermit_macro::system]
#[unsafe(no_mangle)]
pub extern "C" fn sys_console_log_len() -> usize {
	crate::console_log::len()
}

#[hermit_macro::system]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn sys_read_console_log(offset: usize, buf: *mut u8, buf_len: usize) -> i32 {
	if buf_len == 0 {
		return 0;
	}
	...
	let output = unsafe { slice::from_raw_parts_mut(buf, buf_len) };
	let read = crate::console_log::read(offset, output);
	i32::try_from(read).unwrap_or(i32::MAX)
}

#[hermit_macro::system]
#[unsafe(no_mangle)]
pub extern "C" fn sys_set_console_log_capture(enabled: i32) -> i32 {
	crate::console_log::set_capture_enabled(enabled != 0);
	0
}

#[hermit_macro::system]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn sys_append_memory_db(data: *const u8, data_len: usize) -> i32 {
	if data.is_null() {
		return -i32::from(Errno::Inval);
	}

	match crate::drivers::storage::ahci::append_memory_db(data, data_len) {
		Ok(written) => i32::try_from(written).unwrap_or(i32::MAX),
		Err(()) => -i32::from(Errno::Nospc),
	}
}

#[hermit_macro::system]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn sys_read_memory_db(buf: *mut u8, buf_len: usize) -> i32 {
	if buf.is_null() {
		return -i32::from(Errno::Inval);
	}
	...
	match crate::drivers::storage::ahci::read_memory_db(buf, buf_len) {
		Ok(read) => i32::try_from(read).unwrap_or(i32::MAX),
		Err(()) => -i32::from(Errno::Io),
	}
}

#[hermit_macro::system]
#[unsafe(no_mangle)]
pub extern "C" fn sys_erase_memory_db() -> i32 {
	match crate::drivers::storage::ahci::erase_memory_db() {
		Ok(()) => 0,
		Err(()) => -i32::from(Errno::Io),
	}
}
