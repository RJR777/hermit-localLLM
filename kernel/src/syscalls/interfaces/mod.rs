...
pub trait SyscallInterface: Send + Sync {
	...
	fn get_model_ptr(&self) -> *const u8 {
		env::model_info()
			.map(|info| core::ptr::with_exposed_provenance(info.address))
			.unwrap_or(ptr::null())
	}

	fn get_model_len(&self) -> usize {
		env::model_info().map(|info| info.size).unwrap_or(0)
	}
}
