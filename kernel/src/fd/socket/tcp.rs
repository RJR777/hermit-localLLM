...
impl ObjectInterface for Socket {
	...
	async fn connect(&mut self, endpoint: Endpoint) -> io::Result<()> {
		...
		self.with_context(|socket, cx| {
			socket.connect(cx, endpoint, crate::executor::network::get_ephemeral_port())
		})
		.map_err(|_| Errno::Io)?;
	...
	async fn setsockopt(&mut self, opt: SocketOption) -> io::Result<()> {
		if let SocketOption::TcpNoDelay(optval) = opt {
			let mut guard = NIC.lock();
			let nic = guard.as_nic_mut().unwrap();
	...
	async fn getsockopt(&mut self, opt: SocketOption) -> io::Result<SocketOption> {
		if let SocketOption::TcpNoDelay(_) = opt {
			let mut guard = NIC.lock();
			let nic = guard.as_nic_mut().unwrap();
			let socket = nic.get_mut_socket::<tcp::Socket<'_>>(*self.handle.first().unwrap());

			Ok(SocketOption::TcpNoDelay(socket.nagle_enabled()))
		} else {
			Err(Errno::Inval)
		}
	}
