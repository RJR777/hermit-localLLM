...
pub(crate) trait ObjectInterface: Sync + Send {
	...
	#[cfg(any(feature = "net", feature = "virtio-vsock"))]
	async fn setsockopt(&mut self, _opt: SocketOption) -> io::Result<()> {
		Err(Errno::Notsock)
	}

	/// `getsockopt` gets options on sockets
	#[cfg(any(feature = "net", feature = "virtio-vsock"))]
	async fn getsockopt(&mut self, _opt: SocketOption) -> io::Result<SocketOption> {
		Err(Errno::Notsock)
	}
...
pub(crate) fn read(fd: RawFd, buf: &mut [u8]) -> io::Result<usize> {
	...
	let timeout = block_on(async { Ok(obj.read().await.read_timeout().await) }, None)?;
	block_on(async { obj.read().await.read(buf).await }, timeout)
}
...
pub(crate) fn write(fd: RawFd, buf: &[u8]) -> io::Result<usize> {
	...
	let timeout = block_on(async { Ok(obj.read().await.write_timeout().await) }, None)?;
	block_on(async { obj.read().await.write(buf).await }, timeout)
}
