...
pub(crate) fn run() {
	without_interrupts(|| {
		// USB keyboard input is serviced by async tasks. Poll enough ready tasks
		// per scheduler pass that interactive input is not left behind network or
		// storage tasks.
		for _ in 0..32 {
...
pub fn init() {
	#[cfg(feature = "net")]
	// crate::executor::network::init(); // Disabled network init
	#[cfg(feature = "virtio-vsock")]
		...
		if backoff.is_completed() {
			...
			if delay.unwrap_or(10_000_000) > 10_000 {
				...
				let remaining = timeout.map(|duration| {
					let deadline = start + u64::try_from(duration.as_micros()).unwrap();
					deadline.saturating_sub(now)
				});

				// switch to another task
				task_notify.wait(remaining);
