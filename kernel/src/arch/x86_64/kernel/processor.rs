...
/// Current Performance Status. On Intel SpeedStep CPUs bits 15:8 expose the active ratio.
...
static KERNEL_ENTRY_TIMESTAMP: AtomicU64 = AtomicU64::new(0);
...
impl fmt::Display for CpuFrequencySources {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match &self {
			...
			CpuFrequencySources::MsrPerfStatus => write!(f, "MSR Performance Status"),
...
impl CpuFrequency {
	...
	unsafe fn detect_from_msr_perf_status(
		&mut self,
		cpuid: &CpuId<CpuIdReaderNative>,
	) -> Result<(), ()> {
		let feature_info = cpuid.get_feature_info().ok_or(())?;
		if !feature_info.has_eist() {
			return Err(());
		}

		let bus_mhz = cpuid
			.get_processor_frequency_info()
			.map(|info| info.bus_frequency())
			.filter(|&freq| freq > 0)
			.unwrap_or(100);
		let perf_status = unsafe { Msr::new(IA32_PERF_STATUS).read() };
		let ratio = ((perf_status >> 8) & 0xff) as u16;
		let mhz = ratio.checked_mul(bus_mhz).ok_or(())?;

		self.set_detected_cpu_frequency(mhz, CpuFrequencySources::MsrPerfStatus)
	}
	...
	unsafe fn detect(&mut self) {
		...
		unsafe {
			self.detect_from_fdt()
				...
				.or_else(|_e| self.detect_from_msr_perf_status(&cpuid))
...
pub fn reboot() -> ! {
	...
	unsafe {
		let mut reset = Port::<u8>::new(0xcf9);
		reset.write(0x02);
		udelay(10_000);
		reset.write(0x06);
	}
	...
	udelay(1_000_000);
	error!("DIAG: reboot did not complete; halting");
	loop {
		core::hint::spin_loop();
	}
}
...
pub fn shutdown(error_code: i32) -> ! {
	error!(
		"DIAG: shutdown requested with error_code={error_code}; halting forever instead of ACPI poweroff/triple fault"
	);
...
pub fn get_timer_ticks() -> u64 {
	...
	let entry_timestamp = KERNEL_ENTRY_TIMESTAMP.load(Ordering::Relaxed);
	timestamp.saturating_sub(entry_timestamp) / u64::from(get_frequency())
}

pub fn mark_kernel_entry_timestamp() {
	let timestamp = get_timestamp();
	let _ =
		KERNEL_ENTRY_TIMESTAMP.compare_exchange(0, timestamp, Ordering::Relaxed, Ordering::Relaxed);
}
