...
/// The ACPI reset register and value from the FADT.
static RESET_REG: OnceCell<AcpiResetRegister> = OnceCell::new();
...
const GENERIC_ADDRESS_SYSTEM_MEMORY: u8 = 0;
...
fn parse_fadt(fadt: AcpiTable<'_>) {
	...
	let reset_reg_field_address = ptr::from_ref(&fadt_table.reset_reg).addr();
	if reset_reg_field_address < fadt.table_end_address()
		&& (fadt_table.flags & FADT_FLAG_RESET_REG_SUP) != 0
		&& fadt_table.reset_reg.address != 0
	{
		let _ = RESET_REG.set(AcpiResetRegister {
			address_space: fadt_table.reset_reg.address_space,
			bit_width: fadt_table.reset_reg.bit_width,
			address: fadt_table.reset_reg.address,
			value: fadt_table.reset_value,
		});
	}
...
pub fn reboot() -> bool {
	let Some(reset) = RESET_REG.get().copied() else {
		return false;
	};

	unsafe {
		match reset.address_space {
			GENERIC_ADDRESS_IO_SPACE => match reset.bit_width {
				0..=8 => Port::<u8>::new(reset.address as u16).write(reset.value),
				9..=16 => Port::<u16>::new(reset.address as u16).write(u16::from(reset.value)),
				_ => Port::<u32>::new(reset.address as u16).write(u32::from(reset.value)),
			},
			GENERIC_ADDRESS_SYSTEM_MEMORY => match reset.bit_width {
				0..=8 => ptr::with_exposed_provenance_mut::<u8>(reset.address as usize)
					.write_volatile(reset.value),
				9..=16 => ptr::with_exposed_provenance_mut::<u16>(reset.address as usize)
					.write_volatile(u16::from(reset.value)),
				_ => ptr::with_exposed_provenance_mut::<u32>(reset.address as usize)
					.write_volatile(u32::from(reset.value)),
			},
			_ => {
				return false;
			}
