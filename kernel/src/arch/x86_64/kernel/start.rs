...
#[unsafe(no_mangle)]
#[unsafe(naked)]
pub unsafe extern "C" fn _start(_boot_info: *const RawBootInfo, cpu_id: u32) -> ! {
	// boot_info is in the `rdi` register

	// validate signatures
	// `_Start` is compatible to `Entry`
	{
		unsafe extern "C" fn _entry(_boot_info: *const RawBootInfo, _cpu_id: u32) -> ! {
			unreachable!()
		}
		pub type _Start = unsafe extern "C" fn(boot_info: *const RawBootInfo, cpu_id: u32) -> !;
		const _ENTRY: _Start = _entry;
	...
	naked_asm!(
		...
		// Preserve framebuffer information passed by the UEFI loader.
		"mov rax, qword ptr [rip + {loader_fb_base}@GOTPCREL]",
		"mov qword ptr [rax], rdx",
		"mov rax, qword ptr [rip + {loader_fb_stride}@GOTPCREL]",
		...
		loader_fb_base = sym super::LOADER_FB_BASE,
		loader_fb_stride = sym super::LOADER_FB_STRIDE,
