...
pub(crate) fn init() {
	...
	// For Hermit, we scan all 256 buses.
	scan_bus(0..=255, PciConfigRegion::Pci(LegacyPciConfigRegion::new()));
	...
	for bus in bus_range {
		...
		for device in 0..PCI_MAX_DEVICE_NUMBER {
			...
			if device_id != u16::MAX && vendor_id != u16::MAX {
				let pci_device = PciDevice::new(pci_address, pci_config);
				PCI_DEVICES.with(|pci_devices| pci_devices.unwrap().push(pci_device));

				if unsafe { pci_config.read(pci_address, 0x0c) & 0x00800000 != 0 } {
					for function in 1..8 {
