			...
			match map {
				...
				Err(err) => {
					let err_tag = match err {
						MapToError::FrameAllocationFailed => "frame_alloc_failed",
						MapToError::ParentEntryHugePage => "parent_entry_huge_page",
						MapToError::PageAlreadyMapped(_) => "page_already_mapped",
					};
					crate::console::_panic_print(format_args!(
						"paging map failure page={:#x} frame={:#x} flags={:#x} err={}\n",
...
pub(crate) extern "x86-interrupt" fn page_fault_handler(
	...
	error!(
		"Page fault (#PF) at {:p} with error code {:?} (RSVD: {})",
		addr,
		error_code,
		(error_code.bits() >> 3) & 1
	);
