...
#[path = "drivers/console/gop.rs"]
mod framebuffer_console;
#[cfg(target_arch = "x86_64")]
#[path = "drivers/console/font.rs"]
mod framebuffer_font;
pub mod fs;
mod init_cell;
pub mod io;
pub mod mm;
...
#[cfg(target_os = "none")]
// TOTAL PURGE: Uhyve interface removed
#[cfg(test)]
#[cfg(target_os = "none")]
#[unsafe(no_mangle)]
...
extern "C" fn initd(_arg: usize) {
	...
	if env::is_uhyve() {
		...
		info!("Hermit is running on BARE METAL SILICON!");
	}
...
fn boot_processor_main() -> ! {
	...
	// Early diagnostic print to COM1 (0x3F8)
	unsafe {
		let msg = b"CORE_LOCAL_INSTALLED\n";
		for &b in msg {
			while (::x86_64::instructions::port::Port::<u8>::new(0x3f8 + 5).read() & 0x20) == 0 {}
			::x86_64::instructions::port::Port::<u8>::new(0x3f8).write(b);
		}
