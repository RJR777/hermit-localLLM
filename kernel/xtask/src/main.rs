...
pub fn project_root() -> PathBuf {
	if let Some(root) = env::var_os("HERMIT_PROJECT_ROOT") {
	...
	if let Ok(cwd) = env::current_dir() {
		...
		if cwd.join("kernel").join("Cargo.toml").exists() {
			return cwd.join("kernel");
		}
		if let Some(parent) = cwd.parent() {
			if parent.join("kernel").join("Cargo.toml").exists() {
				return parent.join("kernel");
			}
		}
	}
		...
		.and_then(|exe| exe.parent().map(|p| p.to_path_buf()))
	{
		...
		for _ in 0..6 {
			...
			if candidate.join("kernel").join("Cargo.toml").exists() {
	...
	if default.exists() {
		return default.parent().unwrap_or(default).to_path_buf();
	}
