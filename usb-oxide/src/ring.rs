...
impl<H: Dma> Ring<H> {
    pub fn new(host: &H, trb_count: usize) -> Result<Self> {
        let mem = PhysMem::alloc(host, trb_count * core::mem::size_of::<Trb>(), 64)?;
        Ok(Self {
            mem,
            enqueue: 0,
            cycle: true,
...
impl<H: Dma> EventRing<H> {
    pub fn new(host: &H, trb_count: usize) -> Result<Self> {
        let ring = PhysMem::alloc(host, trb_count * core::mem::size_of::<Trb>(), 64)?;
    ...
    pub fn try_dequeue(&mut self) -> Option<Trb> {
        let trb = unsafe {
            let ptr = (self.ring.as_ptr::<Trb>()).add(self.dequeue);
