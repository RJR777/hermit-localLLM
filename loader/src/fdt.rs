...
impl Fdt {
	...
	pub fn framebuffer(mut self, framebuffer: FramebufferInfo) -> FdtWriterResult<Self> {
		let framebuffer_node = self
			.writer
			.begin_node(format!("framebuffer@{:x}", framebuffer.address).as_str())?;
		self.writer
			.property_string("compatible", "simple-framebuffer")?;
		self.writer
			.property_array_u64("reg", &[framebuffer.address, framebuffer.size])?;
		self.writer.property_u32("width", framebuffer.width)?;
		self.writer.property_u32("height", framebuffer.height)?;
		self.writer.property_u32("stride", framebuffer.stride)?;
		self.writer.property_u32("bits-per-pixel", 32)?;
		self.writer.property_string("format", framebuffer.format)?;
		self.writer.end_node(framebuffer_node)?;
...
mod uefi {
	...
	impl super::Fdt {
		pub fn memory_map(mut self, memory_map: &mut impl MemoryMapMut) -> FdtWriterResult<Self> {
			...
			let entries = memory_map.entries().filter(|entry| {
				matches!(
					entry.ty,
