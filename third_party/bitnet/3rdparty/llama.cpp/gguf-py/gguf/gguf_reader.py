...
class GGUFReader:
    ...
    def _build_tensors(self, start_offs: int, fields: list[ReaderField]) -> None:
        ...
        tensor_offsets = [int(field.parts[-1][0]) for field in fields]
        sorted_tensor_offsets = sorted(tensor_offsets)
        ...
        for field in fields:
            ...
            if ggml_type == GGMLQuantizationType.F16:
                ...
                if ggml_type == GGMLQuantizationType.I2_S:
                    offset_value = int(offset_tensor[0])
                    next_offsets = [off for off in sorted_tensor_offsets if off > offset_value]
                    next_data_offs = int(start_offs + next_offsets[0]) if next_offsets else len(self.data)
                    n_bytes = next_data_offs - data_offs
                item_count = n_bytes
                item_type = np.uint8
                if ggml_type == GGMLQuantizationType.I2_S:
                    np_dims = (n_bytes,)
