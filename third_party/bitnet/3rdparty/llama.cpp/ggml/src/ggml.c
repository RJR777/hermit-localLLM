...
static bool gguf_fread_mem_el(const uint8_t * data, size_t size, void * dst, size_t len, size_t * offset) {
...
static bool gguf_fread_mem_str(const uint8_t * data, size_t size, struct gguf_str * p, size_t * offset) {
    p->n    = 0;
    p->data = NULL;

    bool ok = true;

    ok = ok && gguf_fread_mem_el(data, size, &p->n, sizeof(p->n), offset);
    ...
    p->data = ok ? GGML_CALLOC(p->n + 1, 1) : NULL;
    ok = ok && p->data != NULL;

    ok = ok && gguf_fread_mem_el(data, size, p->data, p->n, offset);
...
struct gguf_context * gguf_init_from_buffer(const void * data_ptr, size_t size, struct gguf_init_params params) {
    if (data_ptr == NULL) {
    ...
    {
        if (!gguf_fread_mem_el(data_src, size, &magic, sizeof(magic), &offset)) {
...

    {
        ...
        ok = ok && gguf_fread_mem_el(data_src, size, &ctx->header.version,   sizeof(ctx->header.version),   &offset);
        ok = ok && gguf_fread_mem_el(data_src, size, &ctx->header.n_tensors, sizeof(ctx->header.n_tensors), &offset);
        ok = ok && gguf_fread_mem_el(data_src, size, &ctx->header.n_kv,      sizeof(ctx->header.n_kv),      &offset);
...

    {
        ...
        for (uint64_t i = 0; i < n_kv; ++i) {
            ...
            ok = ok && gguf_fread_mem_str(data_src, size, &kv->key, &offset);
            ok = ok && gguf_fread_mem_el (data_src, size, &kv->type, sizeof(kv->type), &offset);

            switch (kv->type) {
                case GGUF_TYPE_UINT8:   ok = ok && gguf_fread_mem_el(data_src, size, &kv->value.uint8,   sizeof(kv->value.uint8),   &offset); break;
                case GGUF_TYPE_INT8:    ok = ok && gguf_fread_mem_el(data_src, size, &kv->value.int8,    sizeof(kv->value.int8),    &offset); break;
                case GGUF_TYPE_UINT16:  ok = ok && gguf_fread_mem_el(data_src, size, &kv->value.uint16,  sizeof(kv->value.uint16),  &offset); break;
                case GGUF_TYPE_INT16:   ok = ok && gguf_fread_mem_el(data_src, size, &kv->value.int16,   sizeof(kv->value.int16),   &offset); break;
                case GGUF_TYPE_UINT32:  ok = ok && gguf_fread_mem_el(data_src, size, &kv->value.uint32,  sizeof(kv->value.uint32),  &offset); break;
                case GGUF_TYPE_INT32:   ok = ok && gguf_fread_mem_el(data_src, size, &kv->value.int32,   sizeof(kv->value.int32),   &offset); break;
                case GGUF_TYPE_FLOAT32: ok = ok && gguf_fread_mem_el(data_src, size, &kv->value.float32, sizeof(kv->value.float32), &offset); break;
                case GGUF_TYPE_UINT64:  ok = ok && gguf_fread_mem_el(data_src, size, &kv->value.uint64,  sizeof(kv->value.uint64),  &offset); break;
                case GGUF_TYPE_INT64:   ok = ok && gguf_fread_mem_el(data_src, size, &kv->value.int64,   sizeof(kv->value.int64),   &offset); break;
                case GGUF_TYPE_FLOAT64: ok = ok && gguf_fread_mem_el(data_src, size, &kv->value.float64, sizeof(kv->value.float64), &offset); break;
                case GGUF_TYPE_BOOL:    ok = ok && gguf_fread_mem_el(data_src, size, &kv->value.bool_,   sizeof(kv->value.bool_),   &offset); break;
                case GGUF_TYPE_STRING:  ok = ok && gguf_fread_mem_str(data_src, size, &kv->value.str, &offset); break;
                case GGUF_TYPE_ARRAY:
                    {
                        ok = ok && gguf_fread_mem_el(data_src, size, &kv->value.arr.type, sizeof(kv->value.arr.type), &offset);
                        ok = ok && gguf_fread_mem_el(data_src, size, &kv->value.arr.n,    sizeof(kv->value.arr.n),    &offset);
                            ...
                            case GGUF_TYPE_BOOL:
                                {
                                    ...
                                    kv->value.arr.data = GGML_CALLOC(kv->value.arr.n, gguf_type_size(kv->value.arr.type));
                                    ok = ok && gguf_fread_mem_el(data_src, size, kv->value.arr.data, kv->value.arr.n * gguf_type_size(kv->value.arr.type), &offset);
                                } break;
                            case GGUF_TYPE_STRING:
                                {
                                    if (kv->value.arr.n >= SIZE_MAX/sizeof(struct gguf_str)) {
                                    ...
                                    kv->value.arr.data = GGML_CALLOC(kv->value.arr.n, sizeof(struct gguf_str));
                                    for (uint64_t j = 0; j < kv->value.arr.n; ++j) {
                                        ok = ok && gguf_fread_mem_str(data_src, size, &((struct gguf_str *) kv->value.arr.data)[j], &offset);
                                    }
                                } break;
                            case GGUF_TYPE_ARRAY:
                            default: GGML_ABORT("invalid type");
    ...
    if (ctx->header.n_tensors > 0) {
        ...
        for (uint64_t i = 0; i < ctx->header.n_tensors; ++i) {
            ...
            ok = ok && gguf_fread_mem_str(data_src, size, &info->name, &offset);
            ok = ok && gguf_fread_mem_el (data_src, size, &info->n_dims, sizeof(info->n_dims), &offset);
            ok = ok && (info->n_dims <= GGML_MAX_DIMS);

            for (uint32_t j = 0; j < info->n_dims; ++j) {
                ok = ok && gguf_fread_mem_el(data_src, size, &info->ne[j], sizeof(info->ne[j]), &offset);
            }

            ok = ok && gguf_fread_mem_el(data_src, size, &info->type,   sizeof(info->type),   &offset);
            ok = ok && gguf_fread_mem_el(data_src, size, &info->offset, sizeof(info->offset), &offset);
    ...
    if (offset > size) {
        fprintf(stderr, "%s: data offset is past buffer size\n", __func__);
        gguf_free(ctx);
        return NULL;
    }
...

    {
        ...
        for (uint64_t i = 0; i < ctx->header.n_tensors; ++i) {
            ...
            size_t size_cur = ggml_row_size(info->type, ne);
            if (info->type == GGML_TYPE_I2_S || info->type == GGML_TYPE_TL1) {
                size_cur = size_cur / 4 + 32;
            } else if (info->type == GGML_TYPE_TL2) {
                size_cur = (info->ne[0] - 256) * info->ne[1] / 3 * 5 / 8 + 256 * info->ne[1] / 2 * 4 / 8;
                if (size_cur % 32 != 0) {
                    size_cur += 32 - size_cur % 32;
                }
                size_cur += 32;
            }
            const size_t tensor_end = ctx->offset + info->offset + size_cur;
            if (tensor_end < ctx->offset || tensor_end > size) {
                fprintf(stderr, "%s: tensor '%s' data is not within the buffer bounds\n", __func__, info->name.data);
                gguf_free(ctx);
                return NULL;
            }
    ...
    if (params.ctx != NULL) {
        ...
        *params.ctx = ggml_init(pdata);
        ...
        if (!params.no_alloc) {
            data = ggml_new_tensor_1d(ctx_data, GGML_TYPE_I8, ctx->size);
            ok = ok && data != NULL;
            ok = ok && gguf_fread_mem_el(data_src, size, data->data, ctx->size, &offset);
            if (!ok) {
                fprintf(stderr, "%s: failed to read tensor data\n", __func__);
                ggml_free(ctx_data);
                gguf_free(ctx);
...
int gguf_get_tensor_n_dims(const struct gguf_context * ctx, int i) {
    return ctx->infos[i].n_dims;
}

int64_t gguf_get_tensor_ne(const struct gguf_context * ctx, int i, int j) {
    GGML_ASSERT(j >= 0 && j < GGML_MAX_DIMS);
    return ctx->infos[i].ne[j];
}

size_t gguf_get_tensor_size(const struct gguf_context * ctx, int i) {
    const struct gguf_tensor_info * info = &ctx->infos[i];
    const int64_t ne =
        (int64_t) info->ne[0] *
        (int64_t) info->ne[1] *
        (int64_t) info->ne[2] *
    ...
    size_t size = ggml_row_size(info->type, ne);
    if (info->type == GGML_TYPE_I2_S || info->type == GGML_TYPE_TL1) {
        size = size / 4 + 32;
    } else if (info->type == GGML_TYPE_TL2) {
        size = (info->ne[0] - 256) * info->ne[1] / 3 * 5 / 8 + 256 * info->ne[1] / 2 * 4 / 8;
        if (size % 32 != 0) {
            size += 32 - size % 32;
        }
        size += 32;
