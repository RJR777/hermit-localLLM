...
fn cvt(err: i32) -> i32 {
	match err {
		libc::EACCES => ERRNO_ACCES.raw() as i32,
		libc::EBADF => ERRNO_BADF.raw() as i32,
		libc::ENOENT => ERRNO_NOENT.raw() as i32,
		libc::ENOTDIR => ERRNO_NOTDIR.raw() as i32,
		libc::EINVAL => ERRNO_INVAL.raw() as i32,
		libc::EFAULT => ERRNO_FAULT.raw() as i32,
		libc::EIO => ERRNO_IO.raw() as i32,
		libc::ENAMETOOLONG => ERRNO_NAMETOOLONG.raw() as i32,
		libc::ENOMEM => ERRNO_NOMEM.raw() as i32,
		libc::EPERM => ERRNO_PERM.raw() as i32,
		_ => ERRNO_NOSYS.raw() as i32,
	}
}

fn guest_args(module_and_args: &'static [OsString]) -> Vec<&'static OsString> {
	if module_and_args.get(1).is_some_and(|arg| arg == "--") {
		module_and_args[..1]
			.iter()
			.chain(module_and_args[2..].iter())
			.collect()
	} else {
		module_and_args.iter().collect()
	}
}
...
pub(crate) fn init<T: 'static>(
	...
	let guest_args: &'static [&'static OsString] =
		Box::leak(guest_args(module_and_args).into_boxed_slice());
	info!("WASI guest argv: {guest_args:?}");
		...
		.func_wrap(
			...
			|mut caller: Caller<'_, _>, argv_ptr: i32, argv_buf_ptr: i32| {
				...
				if let Some(Extern::Memory(mem)) = caller.get_export("memory") {
					...
					for (i, element) in guest_args.iter().enumerate() {
						let _ = mem.write(
							caller.as_context_mut(),
			...
			|mut caller: Caller<'_, _>,
			 ...
			 fd_ptr: i32| {
				...
				if let Some(Extern::Memory(mem)) = caller.get_export("memory") {
					...
					let guest_path = std::str::from_utf8(&path).unwrap();
					let path = if guest_path.starts_with('/') {
						guest_path.to_owned()
					} else {
						"/".to_owned() + guest_path
					};
					...
					if (fs_rights_base & RIGHTS_FD_WRITE) != 0
						|| oflags.intersects(Oflags::OFLAGS_CREAT | Oflags::OFLAGS_TRUNC)
					{
						flags |= libc::O_RDWR;
					} else {
						flags |= libc::O_RDONLY;
					...
					c_path[..path.len()].copy_from_slice(path.as_bytes());
					{
						...
						if raw_fd < 0 {
							...
							error!("WASI path_open failed: path={path:?} flags={flags:#x} errno={errno}");
							return cvt(errno);
						}
						info!("WASI path_open: path={path:?} fd={raw_fd} flags={flags:#x}");
						let mut guard = FD.lock().unwrap();
	...
	linker
		.func_wrap(
			...
			|mut caller: Caller<'_, _>, fd: i32, path_ptr: i32, path_len: i32| {
				info!("WASI fd_prestat_dir_name fd={fd} path_len={path_len}");
				let guard = FD.lock().unwrap();
				if fd < guard.len().try_into().unwrap()
	...
	linker
		.func_wrap(
			...
			"fd_fdstat_set_flags",
			|fd: i32, fdflags: i32| {
				info!("WASI fd_fdstat_set_flags fd={fd} flags={fdflags:#x}");
				ERRNO_SUCCESS.raw() as i32
			},
		)
		.unwrap();
	...
	linker
		.func_wrap(
			...
			move |mut caller: Caller<'_, _>, number_args_ptr: i32, args_size_ptr: i32| {
				let nargs: u32 = guest_args.len().try_into().unwrap();
				info!("WASI args_sizes_get nargs={nargs}");
				...
				if let Some(Extern::Memory(mem)) = caller.get_export("memory") {
					...
					.map_err(|err| error!("WASI args_sizes_get argc write failed: {err}"));

					let nargs_size: u32 = guest_args
						.iter()
						.fold(0, |acc, arg| {
						...
						.try_into()
						.unwrap();
					info!("WASI args_sizes_get argv_buf_size={nargs_size}");
					let _ = mem.write(
						caller.as_context_mut(),
						args_size_ptr.try_into().unwrap(),
						nargs_size.as_bytes(),
					)
					.map_err(|err| error!("WASI args_sizes_get size write failed: {err}"));
