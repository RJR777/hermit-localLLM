...
impl KernelSrc {
	...
	fn build(self) {
		...
		cargo
			...
			.env("HERMIT_PROJECT_ROOT", &self.src_dir)
			.arg("run")
			.arg("--package=kernel-xtask")
		...
		if env::var_os("HERMIT_VERBOSE_BUILD").is_some() {
			eprintln!("$ {cargo:?}");
		}
		...
		self.rerun_if_changed_path(&self.src_dir.join("src"));
		self.rerun_if_changed_path(&self.src_dir.join("Cargo.lock"));
		self.rerun_if_changed_path(&self.src_dir.join("build.rs"));
		...
		if let Some(workspace_root) = self.src_dir.parent() {
			self.rerun_if_changed_path(&workspace_root.join("usb-oxide/src"));
			self.rerun_if_changed_path(&workspace_root.join("usb-oxide/Cargo.toml"));
			self.rerun_if_changed_path(&workspace_root.join("usb-oxide/build.rs"));
			self.rerun_if_changed_path(&workspace_root.join("kernel/src"));
			self.rerun_if_changed_path(&workspace_root.join("kernel/Cargo.toml"));
			self.rerun_if_changed_path(&workspace_root.join("kernel/Cargo.lock"));
		}
	...
	fn rerun_if_changed_path(&self, path: &Path) {
		if path.exists() {
